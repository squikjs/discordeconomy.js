...
client.on("message", (message) => {
  ...
  if (command === "work") {
    ...
    //sample TRANSFER/GIVE command
    if (!args[0] || !args[1] || !args[2])
      return message.reply(
        `wrong syntax!\nCorrect Syntax is - \`${prefix}${command} [coins/gem] [amount] [user]\``
      );
    if (args[0].toLowerCase() !== "coins" && args[0].toLowerCase() !== "gem")
      return message.reply(
        `wrong syntax!\nCorrect Syntax is - \`${prefix}${command} [coins/gem] [amount] [user]\``
      );
    ...
    if (args[0].toLowerCase() === "gem") {
      if (gem.fetch(message.author.id) < amount)
        return message.reply("you don't have enough gem!");
    ...
    //sample COINFLIP command
    if (!args[0])
      return message.reply(
        `wrong syntax!\nCorrect Syntax is - \`${prefix}${command} [amount] [head/tail]\``
      );
    if (args[1].toLowerCase() !== "head" && args[1].toLowerCase() !== "tail")
      return message.reply(
        `wrong syntax!\nCorrect Syntax is - \`${prefix}${command} [amount] [head/tail]\``
      );
    ...
    //sample SLOTS command
    if (!args[0] || isNaN(parseInt(args[0])) || parseInt(args[0]) <= 0)
      return message.reply(
        "wrong syntax\nCorrect syntax - " + `\`${prefix}${command} [amount]\``
      );

    const slots = balance.slots(message.author.id, parseInt(args[0]), [
      "🍎",
      "🍌",
      "🍿",
      "🍨",
      "🍇",
    ]); //in the 3rd paramter, you can put array of objects
