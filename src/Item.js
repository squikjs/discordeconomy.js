...
class Item {
    constructor(item) {
        if(!item) throw new Error('Item must be specified! Example - new Item(\'gem\')');
        this.item = item;
    }

    /**
     * 
     * @param {*} id ID of the user to add the items to
     * @param {*} amountInitial Amount of items to add
     */
    ...
    leaderboard = (Limit) => {
        ...
        const lb = db.all()
            ...
            .map((user, position) => `#${position + 1} <@!${user.ID}>: ${user.data[`${this.item}`]} ${this.item}`)
